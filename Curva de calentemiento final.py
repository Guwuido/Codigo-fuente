...
class SimuladorCalentamientoGUI:
    ...
    def calcular_temperatura_saturacion(self, presion_kpa):
        """Calcula la temperatura de saturación del agua según presión (kPa) usando el modelo tipo Wagner/IAPWS."""
        PSTAR = 1.0  # MPa
        TSTAR = 1.0  # K
        ...
        beta = (presion_kpa / 1000.0 / PSTAR) ** 0.25
        E = beta ** 2 + n[3] * beta + n[6]
        F = n[1] * beta ** 2 + n[4] * beta + n[7]
        G = n[2] * beta ** 2 + n[5] * beta + n[8]
        D = (2 * G) / (-F - (F ** 2 - 4 * E * G) ** 0.5)
        T_s_k = TSTAR * (
            n[10] + D - ((n[10] + D) ** 2 - 4 * (n[9] + n[10] * D)) ** 0.5
        ) / 2

        return T_s_k - 273.15  # Retorna en °C

    def calcular_entalpia_vaporizacion(self, temperatura):
        """Calcula la entalpía de vaporización con modelo empírico ajustado."""
        T_kelvin = temperatura + 273.15
        T_critica = 647.0969

        if T_kelvin >= T_critica:
            return 0.0
        h_lv_kj_kg = 2256.4 / ((T_kelvin / T_critica) ** (1 - 0.57665623))
        return h_lv_kj_kg * 1000.0  # en J/kg
